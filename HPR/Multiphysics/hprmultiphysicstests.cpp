#include "hprmultiphysicstests.h"

HPRMultiphysicsTests::HPRMultiphysicsTests()
{
}

void HPRMultiphysicsTests::solveBartonTest1(int cellCount, int reinitialisationFrequency)
{
    double cellSpacing = 1.0 / cellCount;

    vector<HPRMultiphysicsStateVector> initialCells(cellCount);
    ...
    for (int i = 0; i < cellCount; i++)
    {
        if (i <= (0.5 * cellCount))
        {
            initialCells[i] = HPRMultiphysicsStateVector(0.999, 0.0, 0.5, 1.0, leftDistortionTensor, pow(10.0, -3.0), rightDistortionTensor, 0.0, hyperelasticMaterialParameters,
                                                         hyperelasticMaterialParameters, materialParameters, materialParameters);
        }
        else
        {
            initialCells[i] = HPRMultiphysicsStateVector(0.001, 0.0, 0.0, 0.0, leftDistortionTensor, pow(10.0, -3.0), rightDistortionTensor, 0.0, hyperelasticMaterialParameters,
                                                         hyperelasticMaterialParameters, materialParameters, materialParameters);
        }
    }
...
void HPRMultiphysicsTests::solveZhangTest(int cellCount, int reinitialisationFrequency)
{
    double cellSpacing = 1.0 / cellCount;
    ...
    for (int i = 0; i < cellCount; i++)
    {
        if (i <= (0.5 * cellCount))
        {
            initialCells[i] = HPRMultiphysicsStateVector(0.999, 2.0, 0.0, 0.1, leftDistortionTensor, 0.0, rightDistortionTensor, 0.0, hyperelasticMaterial1Parameters,
                                                         hyperelasticMaterial2Parameters, material1Parameters, material2Parameters);
        }
        else
        {
            initialCells[i] = HPRMultiphysicsStateVector(0.001, 0.0, -0.03, -0.01, leftDistortionTensor, 0.0, rightDistortionTensor, 0.0, hyperelasticMaterial1Parameters,
                                                         hyperelasticMaterial2Parameters, material1Parameters, material2Parameters);
        }
    }
...
void HPRMultiphysicsTests::outputSolution(vector<HPRMultiphysicsStateVector> solution)
{
