...
void HPRIntermediateTests::solve2DZhangTest(int cellCount, int reinitialisationFrequency)
{
    double cellSpacing = 1.0 / cellCount;
    ...
    for (int i = 0; i < cellCount; i++)
    {
        for (int j = 0 ; j < cellCount; j++)
        {
            if (sqrt(((i - (0.5 * cellCount)) * (i - (0.5 * cellCount))) + ((j - (0.5 * cellCount)) * (j - (0.5 * cellCount)))) <= (0.2 * cellCount))
            {
                initialCells[i][j] = HPRIntermediateStateVector(0.999, 2.0, 0.0, 0.1, leftDistortionTensor, 0.0, rightDistortionTensor, 0.0, hyperelasticMaterial1Parameters,
                                                                hyperelasticMaterial2Parameters, material1Parameters, material2Parameters);
            }
            else
            {
                initialCells[i][j] = HPRIntermediateStateVector(0.001, 0.0, -0.03, -0.01, leftDistortionTensor, 0.0, rightDistortionTensor, 0.0, hyperelasticMaterial1Parameters,
                                                                hyperelasticMaterial2Parameters, material1Parameters, material2Parameters);
            }
        }
    }
...
void HPRIntermediateTests::solve2DZhangTest2(int cellCount, int reinitialisationFrequency)
{
    double cellSpacing = 1.0 / cellCount;
    ...
    for (int i = 0; i < cellCount; i++)
    {
        for (int j = 0; j < cellCount; j++)
        {
            initialCells[i][j] = HPRIntermediateStateVector(0.001, 0.0, 0.0, 0.0, distortionTensor, 0.0, distortionTensor, 0.0, hyperelasticMaterial1Parameters,
                                                            hyperelasticMaterial2Parameters, material1Parameters, material2Parameters);
        }
    }
    ...
    for (int i = (0.4 * cellCount); i < (0.5 * cellCount); i++)
    {
        ...
        {
            initialCells[i][j] = HPRIntermediateStateVector(0.999, 0.0, 800.0, 0.0, distortionTensor, 0.0, distortionTensor, 0.0, hyperelasticMaterial1Parameters,
                                                            hyperelasticMaterial2Parameters, material1Parameters, material2Parameters);
        }
    }
    ...
    for (int i = (0.5 * cellCount); i < (0.6 * cellCount); i++)
    {
         ...
         {
             initialCells[i][j] = HPRIntermediateStateVector(0.999, 0.0, 0.0, 0.0, distortionTensor, 0.0, distortionTensor, 0.0, hyperelasticMaterial1Parameters,
                                                             hyperelasticMaterial2Parameters, material1Parameters, material2Parameters);
         }
    }

    outputSolution2D(HPRSecondOrderSolver::solve2D(initialCells, cellSpacing, 0.8, 6.5 * pow(10.0, -4.0), 0.0, 0, 0, reinitialisationFrequency, material1Parameters, material2Parameters));
}

void HPRIntermediateTests::solve2DZhangElasticPlasticTest(int cellCount, int subcyclingIterations, int reinitialisationFrequency)
{
    double cellSpacing = 10.0 / cellCount;

    vector<vector<HPRIntermediateStateVector> > initialCells(cellCount, vector<HPRIntermediateStateVector>(cellCount));
    HyperelasticMaterialParameters hyperelasticMaterialParameters(2.71, 6.22, 3.16, 9.0 * pow(10.0, -4.0), 300.0, 1.0, 3.577, 2.8);
    HPRMaterialParameters materialParameters("GodunovRomenski", false, true, 8.31445985, 2.71, 73.0, 300.0, 300.0, 9.0 * pow(10.0, -4.0), 1.0, 0.0, 5.037, 1.0, 1.338, 0.0, 3.16,
                                             pow(10.0, -9.0), 0.0, 24.8 * pow(10.0, 9.0), 0.75, 0.2976 * pow(10.0, 9.0), 1.338, 2.0, 1.0, 3.577, 2.088);
    ...
    for (int i = 0; i < cellCount; i++)
    {
        for (int j = 0; j < cellCount; j++)
        {
            if (sqrt(((i - (0.5 * cellCount)) * (i - (0.5 * cellCount))) + ((j - (0.5 * cellCount)) * (j - (0.5 * cellCount)))) <= (0.2 * cellCount))
            {
                initialCells[i][j] = HPRIntermediateStateVector(0.999, 0.75, 0.0, 0.0, leftDistortionTensor, 0.0, rightDistortionTensor, 0.0, hyperelasticMaterialParameters,
                                                                hyperelasticMaterialParameters, materialParameters, materialParameters);
            }
            else
            {
                initialCells[i][j] = HPRIntermediateStateVector(0.001, -0.75, 0.0, 0.0, leftDistortionTensor, 0.0, rightDistortionTensor, 0.0, hyperelasticMaterialParameters,
                                                                hyperelasticMaterialParameters, materialParameters, materialParameters);
            }
        }
    }

    outputSolution2D(HPRSecondOrderSolver::solve2D(initialCells, cellSpacing, 0.8, 0.3, 0.0, 0, subcyclingIterations, reinitialisationFrequency, materialParameters, materialParameters));
}

void HPRIntermediateTests::solve2DZhangElasticPlasticTest2(int cellCount, int subcyclingIterations, int reinitialisationFrequency)
{
    double cellSpacing = 1.0 / cellCount;
    ...
    HyperelasticMaterialParameters hyperelasticMaterial2Parameters(0.001, 6.22, 3.16, 9.0 * pow(10.0, -4.0), 300.0, 1.0, 3.577, 2.088);
    ...
    outputSolution2D(HPRSecondOrderSolver::solve2D(initialCells, cellSpacing, 0.8, 1.0 * pow(10.0, -4.0), 0.0, 0, subcyclingIterations, reinitialisationFrequency, material1Parameters,
                                                   material2Parameters));
}
